...
slug: tear-sheet
version: 0.1.2
summary: "公司一页纸：套件里最快回客的件——跑一次出一页，条条带公开披露锚，查不到就写查不到。"
description: "给你一家公司（名或代码），产出一页速览：业务、财务关键值（TTM）、股权与变动、风险线索、近期披露——每行可回指公开披露，取不到的项如实标「未检到」。 默认只走公开数据通道，需深研时先报预计耗时与消耗、经你确认再发起；机器拦无锚数字与结论。 适合：初次接触一家公司、会前速备、快速回客。不适合：深度尽调（转 dd-checklist）、投资意见、把一页纸当报告卖。"
tags: [投研, 一页纸, A股, 回客件]
license: MIT
agent_created: true
metadata:
