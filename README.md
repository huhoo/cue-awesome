...
| [`tear-sheet/`](tear-sheet/) | tear-sheet | 0.1.0 | Company tear-sheet (client meeting prep): ≤5 subjects + context → five fixed sections, every line disclosure-anchored, header carries asof + channel usage; zero deep-research by default; opinion slots always `[待人工]`. **In verification: adversarial final review not passed, no end-to-end run.** |
| [`sector-overview/`](sector-overview/) | sector-overview | 0.1.0 | Industry cycle briefing (six sections): every trend judgement must sit in the same line as its comparable-series anchor, else the verbatim "no comparable series, no cycle judgement" applies; policy timeline uses three-part statute anchors. **In verification: docs + machine-check spec ready; final review and live run pending.** |
...
> **Status.** Four financial-research skills exist in this repository: `cn-earnings-note` (v0.3.3, P1 end-to-end verified, shipped); `catalyst-calendar`, `tear-sheet` and `sector-overview` (all v0.1.0) are **in verification** — code and machine checks in-repo, adversarial final review and end-to-end runs not yet done. The rest is scoped direction **not built and not available**. **Except `cn-earnings-note`, which has completed P1 end-to-end runs (acceptance records in its CHANGELOG Verified); design values remain for all other scoped-but-unbuilt items.** Nothing in this roadmap is promised as shipped beyond what the Skills table above lists.

### Built — the vanguard (4 skills, 3 in verification)
...
- **Tear-sheet — [`tear-sheet/`](tear-sheet/)** (v0.1.0, in verification): client-ready one-pager, five fixed sections, line-level anchors.
- **Sector overview — [`sector-overview/`](sector-overview/)** (v0.1.0, in verification): trend judgements require same-line comparable-series anchors.
