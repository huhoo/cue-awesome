...
> **Status.** Four financial-research skills exist in this repository: `cn-earnings-note` (v0.3.3, P1 end-to-end verified, shipped); `catalyst-calendar`, `tear-sheet` and `sector-overview` (all v0.1.0) passed adversarial final review (2026-09-21) but have **no end-to-end runs yet** — treat first real subjects as unverified; catalyst-calendar additionally completed a zero-billing smoke run (2 subjects, 90-day window, direct domains only). The rest is scoped direction **not built and not available**. **Except `cn-earnings-note`, which has completed P1 end-to-end runs (acceptance records in its CHANGELOG Verified); design values remain for all other scoped-but-unbuilt items.** Nothing in this roadmap is promised as shipped beyond what the Skills table above lists.

### Built — the vanguard (4 skills, 3 reviewed & pending live runs)
