...
version: "0.3.8"
summary: "A股/港股财报深度点评初稿：四期差异、科目勾稽、预告对照；数字行证据级标注逐条机检（95% 汇总阈值），评级判断位 [待人工]。"
description: "给你一家公司 + 报告期，产出八节点评初稿：四期差异、科目勾稽、指引与兑现对照、盈利质量分桶、预测脚手架。四道机检的如实范围：①AI 声明与 [待人工] 就位；②带单位数字行须有证据级标注，逐条拦并设 95% 汇总覆盖率阈值；③账本数值对象带口径 basis 字段；④合规最小集词表。--sources 只在给出时校验 JSONL 形制与文中自写 [S] 可解析，不逐条要求带锚、不回查披露原文；「同比/单季」属出稿纪律，靠人守。适合：财报后复盘、投研底稿、晨会。不适合：出具评级（判断位 [待人工]）、监管申报、无公开披露渠道的公司。"
...
> 四条门禁由 `scripts/check_note.py` 机检（声明与 [待人工] 就位 / 数字行证据级标注逐条 + 95% 汇总覆盖率 / 账本 basis 字段 / 合规词表）；「同比、单季」这类口径标注是**出稿纪律**，机器只核账本数值对象有没有 basis 字段，措辞是否属实归人工。数据骨架是 Cue 三通道，不是自建爬虫层。本 skill 自足运行，不依赖仓内其他 skill 在场。
...
四道：①AI 声明与 [待人工] 就位 ②数字行证据级标注（逐条拦 + 95% 汇总覆盖率阈值）③账本数值对象带 basis 口径字段 ④合规最小集词表。FAIL 则回改后复跑，通过后才把 note 交给用户。送审场景加 `--audit-report <path>` 出「送审就绪度」附录（禁词点名/待人工计数/缺数清单/声明核验/送审包清单），该报告不构成合规意见。
