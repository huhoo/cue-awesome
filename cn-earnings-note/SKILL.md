...
## 3. 管线（`+recall` 续账入口 → 五段 + 收尾门禁；续跑=读表起手+跑前核对机检要点）
...
- **证据层（零计费）**：本节及 `+parse` 每笔域直查/解析的原始返回落 `evidence/` 并在台账记 sha 行——命名/登记/抽查规格=`references/evidence-format.md`（套件三件通用，此处只挂义务不复述契约）。
...
- omni 解析产物同样落 `evidence/` 存快照（解析正文+operation 元数据），sha 台账行同 §3.2 纪律。
...
├── evidence/        证据层快照（命名/sha 台账/抽查清单位=`references/evidence-format.md`，零计费）
...
- A股本土化实践参考：道以研究院 `dao-financial-services` v0.1.9 功能包（MIT；CAS 口径、评级用语白名单、[待人工] 与投毒防御等实践的本土化形态见于该功能包）。查阅方式（公开可核）：功能包下载 <https://hzddyy.com/stdd/api/download/dao>。
