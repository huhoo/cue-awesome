...
version: 0.3.5
summary: "A股/港股财报深度点评初稿：四期差异、科目勾稽、预告对照；数字行可回查覆盖率按 95% 阈值机检，评级判断位 [待人工]。"
description: "给你一家公司 + 报告期，产出八节点评初稿：四期差异、科目勾稽（收入到现金）、指引与兑现对照、盈利质量分桶、预测脚手架。四道机检：AI 声明与 [待人工] 就位、数字行可回查覆盖率（阈值 95%，未覆盖进缺数清单，不逐数必拦）、口径纪律（同比/单季标注）、合规词表最小集；送审场景另出就绪度附录。一致预期只引公开汇总层并自声明层级。适合：财报后复盘、投研底稿、晨会。不适合：出具评级（判断位 [待人工]）、监管申报、无公开披露渠道的公司。"
