...
### 0.3.9 — 2026-09-26（M126：把「带单位数字行」限缩到机器真认的单位集）

- 上一版的「带单位数字行须有证据级标注」仍是大话：`check_note.py:34` 的 NUM_UNIT_RE 只认 **%／percent／亿元／万元／倍／个百分点**，「营业收入 20,000千元」无标注实测 **exit 0（数字行 0/0）**，而「2万元」无标注 **exit 1**。SKILL summary/description/门禁句/四道清单与 README 门禁行共五处同治为「命中机器单位集（%／亿元／万元／倍／个百分点）的数字行」，并明写千元/元/股不在集内、机器不拦。
- 本条只限缩措辞，**不扩机器**：把单位集补全属机器面改动，另记 BACKLOG 候批（P-25），与文案单解耦。上一条 0.3.8 的措辞按「dated 条目不回改」保留原样，误差以本条收口。
