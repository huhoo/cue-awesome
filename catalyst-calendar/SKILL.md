...
1. **输出是 AI 初稿**：标题后前 5 行内含 AI 声明行（机检①）。**评级概念零容忍是机检事实**：`买入/增持/减持/持有/推荐/目标价` 等词在正文（非引用区/非来源附录）出现即 FAIL——本件比姊妹件更严，没有「[待人工] 留白位」这回事，评级在这里根本不适用。
...
4. **无锚即删条**：数字与日期回指不了 `sources` 的条目，直接从日历删除并在台账记弃条原因；**日期一律 `YYYY-MM-DD` 全格式**（机检④），窗口外但确有必要的条目同行注「窗口外余档」（无注即 FAIL——窗外是日期幻觉高发口）。
...
- `+derive`：法定期限类仅对**适用条件在法条锚内可判**者推导（如定期报告披露期限条款含报告期类型与主体类别），**每条挂 `statute` 当场取回的法条原文锚**；规则文本不写死在日历里，推导依据入 sources；条件不可判的期限（如未分主体的快报类）拒推不落行。
- `+build`：按主体节组装 `calendar.md`（日期正排表：日期|事件|类型|状态|来源锚）+「未来 30 天要盯」摘要段（**只列，不评**）+「已发生动态」附注区（进度类披露，带锚不占行）+ 附录来源索引（全锚可解析）。
- `+check`（不过不交付）：
  ```bash
  python3 scripts/check_calendar.py <calendar.md> --sources <sources.jsonl> --window <YYYY-MM-DD~YYYY-MM-DD>
  ```
  **`--window` 必传**（=输入契约定稿后的窗口字面值；checker 禁读墙钟——「今起 90 天」只是输入缺省，进入机检前必须落为显式窗口，否则历史产物隔天重跑结论漂移）。机检要点：①声明行 ②五列表头状态机+数据行逐行列级锚校验（公告索引号/函件号规范式/URL/`conv_id`+路径；事件与状态列数字不得冒充锚）③禁词表+**评级词零容忍** ④日期全格式、按主体节非降序、窗外无注 FAIL ⑤正文锚↔sources 双向可解析+类型×锚型映射 ⑥主体数 ≤10。FAIL 回改复跑。

## 4. 输出契约

```
calendar-<日期>-<n>主体/
├── calendar.md      交付物：声明行+主体节（正排表）+30 天要盯段+已发生动态附注区+附录来源
