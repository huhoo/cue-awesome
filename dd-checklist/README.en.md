...
- **Still not done**: the only measured instance is this one — a single subject, a single window (12m), a single purpose tier (investment); the `credit` and `mna` tiers and wider windows are unrun, multi-subject work is refused by contract, and the P2 listing stays frozen with push and publication owned by the Owner. Outside this back-filled instance, no other "N items found" number appears anywhere in this README.
