...
- **Reviewed twice**: both rounds returned BLOCK first (M63: 4 blockers; M70: 4 blockers — the under-reporting hole found by lead, plus reverse counting, whole-chain rewriting, statute record borrowing and bag-of-words declarations), each fixed question-by-question against §v2, and the M76 re-review is **LGTM**; **no end-to-end run against a real subject has happened** (a real run awaits authorization, same discipline as its sibling packages).
- **Not done**: no end-to-end run against a real single subject has happened (third exit condition still open), so not one "N items found" sentence in this README is a measured result.
