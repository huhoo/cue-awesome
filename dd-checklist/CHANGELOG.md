...
### Fixed（M101 复审＝BLOCK 但零新题号：X27/X28/X29 三格按审方闭包一次封全，版本位不动）

审方这次没出表外形状：三个根因都落在 M99 三态表的同三格里，本轮是收口最后一轮，不是又一轮逐形加门。一处实现对应一条 root rule，不加诊断码、不动证据 schema、不给"不该封"列加闸。

- **§v2-16① 五列账是固定文法的一张表（X27 闭包）**：表头须**恰五列且顺序逐字**为 `| 类目 | 所需域/工具 | 本次实际调用 | 结果 | 注记 |`——旧实现 `set(cells) >= wanted` 放行换序与第六列；分隔行须与表头**同列数**；数据行**读满五列**（第六格不再被静默吃掉）；一交付**恰一个表头**，`header_seen` 不因第二张同名表头重置。四种形状任一即 `DD-COVERAGE`，且未认表时散落的九行管道行仍按"表身份未成立"报，不猜成合法账。
- **§v2-16② 三角第三条边＝域相等（X28 闭包）**：账内每类「本次实际调用」须**等于**该类流水的 `domain` 集——旧实现只判"两名字各自都在合法枚举里"，于是 `regulatory_cn` 的流水配 `entity_data` 的账静默过关；现为 `DD-OMISSION`。只判结构相等，不判来源真假（§7.8 措辞不变）。
- **§v2-16③ 围栏词法按 CommonMark、且掩码共享到所有结构入口（X29 闭包）**：`fence_mask()` 重写——开栏=至多 3 个前导空格 + 至少 3 个同字符（反引号围栏 info string 内不得再含反引号），闭栏=同字符且**长度 ≥ 开栏**（四反引号合法闭合三反引号块，旧实现把后文一直遮住）；四空格缩进的 ``` 是缩进代码行、不是开栏（旧实现据此把伪开栏后的第二条申报掩掉）。同一份正文掩码现在供**独立 `references/coverage-map.md` 账表、每节足行、标题后前 5 行的 AI 初稿声明窗口**共用（审方点名三处泄漏，逐处收口；足行旧实现"先掩后全文 `finditer`"）；**红线仍吃原始全文**，围栏不豁免。
- 夹具：主集仍 **60 坏 + 10 正路 = 70** 不变，控制样 **19 枚 → 31 枚**（题单 M101 节点名的 12 枚全数落地）。生成器断言升级时按盘上实况写：题单以行内代码点名的控制共 20 枚，断言的是**点名→在盘**单向（缺席即 exit 2 不出货），另有 11 枚历史控制题单未点名、只随 runner 复跑——**"31↔31 全等"从题单推不出来，我没有把它写成通过条件，也没有把它当成已满足的宣称**。
- 双向反证（`gtm/scripts/dual_gate.py`，旧门取 `git show HEAD:` 版，同一份夹具）：**十二枚里 11 枚翻转**——五枚账法样与 `ctl-flow-ledger-domain-mismatch`、`ctl-fence-four-space-not-open`、`ctl-ledger-entirely-fenced`、`ctl-fence-authority-only` 旧门 exit 0 零诊断（洞在，与题单记载的 M101 现值逐枚同向）→ 新门 exit 1 且精确码集；两枚反向形状 `ctl-fence-longer-close-good`、`ctl-fence-domain-footer-example-good` 旧门 exit 1 单码 `DD-COVERAGE`（过严误杀）→ 新门 exit 0。**1 枚常驻不翻转**：`ctl-fence-redline-still-scanned` 旧门已正确 FAIL `DD-REDLINE`，它专防"修围栏时把红线也接进掩码"。主集 70/70、控制 31/31 exact-set 全绿，幂等两跑夹具树 hash 同值。
- **一处自曝（本轮我自己引入、被 runner 照出）**：M102 控制块里我把七列风险表的模块常量 `HDR`/`SEP` 直接重新赋值成五列账表头，污染了其后所有由 `report()` 生成的样（含两枚此前已绿的 `ctl-declaration-missing`、`ctl-overclaim-row-missing`），首轮 runner 报 14 枚控制 FAIL。修法不是放宽判卷，而是给五列账另立 `LEDGER_HDR`/`LEDGER_SEP`/`LEDGER_ROW_EQ` 并由 `cov()` 单源引用，两张表的字面从此不可能互相顶名。
- **真实两笔与本刀冲突，报 lead 裁，我不改历史运行件**：m79/m88 的 `references/coverage-map.md` 里「合规与处罚」「财务与披露质量」两行的「本次实际调用」各只写**一个**域，而这两类流水的 `domain` 集各有**两个**域。按 §v2-16② 的"相等"口径，新门对两笔各多出两项 `DD-OMISSION`：m79 旧门 exit 0 → 新门 exit 1；m88 旧门 exit 1 单码 `DD-COVERAGE` → 新门 `DD-COVERAGE + DD-OMISSION`（其余诊断一字未增）。两种收法都成立且我不能替审方选：(a) 若该列语义是"该行结果所由得出的那个域"，则 §v2-16② 该写成"账内域须落在该类流水域集内 + 流水域须在账内某列留痕"；(b) 若该列语义是"本次该类调用过哪些域"，则两笔历史件确实漏记，须补记或另立 Known-issue。**我按题面写了"相等"，没有为了让 m79/m88 跑绿把闸降成子集**（题单明令不许降 spec 迁就现状）。本包对外文案未引用这两笔的逐类码集，故此项不构成公开面失实；收窄句仍指 `README.md`。
- 文档同刀校准（先修实现、后改宣称）：§5 新增 §v2-16 三段；§6 码表 `DD-COVERAGE` / `DD-OMISSION` 两行纯追加（旧行文本整段保留于新行，粗体标记逐行成对已机检）；§6 判卷口径段的控制样描述**去枚数化**（C-19：活计数只留 runner 一处权威）；§7 第 11 格改写为"围栏词法只算一次、掩码共享到所有结构入口、红线唯一不接掩码"。

### Fixed（M100 摊牌轮：七条结构根因一单合治，版本位不动）

契约 §v2-15 的七条一次落进门，一条根因一处实现；"不该封"列（语义真伪、整链重写、`beyond` 自然语言等价、`无工具项: K`、可达面）不加机器闸，只在 §7 加一格人审：

- **① X26 账龄三段核等值**：`disclosure_cn` 须恰为 `365d`——原实现只挡写高（`cap > 365`），`1d` 静默过关；现不等于即 `DD-COVERAGE`。既有 730d 控制样不退化。
- **② X27 九类账是一张封闭表**：先认表（五列表头 + 分隔行）再认类；缺表头/缺分隔行/出现第十类/未认表就吃管道行，全部 `DD-COVERAGE`。同根控制 `ctl-ledger-no-header`。
- **③ X28 流水↔账↔快照三角闭合**：流水八字段齐、`hits` 为非负整数、`domain`↔快照文件域前缀互证；**流水有该类调用而账抹成「无工具/未发起」= `DD-OMISSION`**（旧实现让无工具行免查流水）；长窗申报的触发源不再单采账表，并入流水实际调用域——四类全洗 + 删申报同时报 `DD-OMISSION` 与 `DD-COVERAGE`。控制样 `ctl-flow-missing-hits`、`ctl-flow-domain-snapshot-mismatch`、`ctl-flow-age-trigger-launder`。措辞守 §7.8：只证结构与哈希自洽，不宣称挡整链重写。
- **④ X29 围栏语境**：新增 `fence_mask()` / `body_indices()` / `section_range()`，标题识别、节内取行、申报计数、七列表提取、H1 与回显与主体计数**共用同一份"可交付正文"**——围栏内的 `##`、申报行、管道行是示例文本；红线仍全篇扫四件，不因围栏豁免。正路 `good-X29-fenced-heading-example`（真实唯一节 + 节内围栏只放同名示例标题）必须 exit 0，防"见围栏一律拒产"的恒红假修。
- **⑤ X30 输入权威单值**：`--subject` 改 `fullmatch` 二选一（含两主体的串不再抓首个），H1 与四条参数回显各须恰一个，禁 first/last-wins；控制样 `ctl-input-double-echo`、`ctl-input-double-h1`；纯代码正路 `good-subject-code-only` 不受影响。
- **⑥ X31 足行按表节唯一**：每个含七列表的节内「本节检索域」恰一行——同节堆两行竞争、或拿别节那行抵本节均 `DD-COVERAGE`；合法紧凑一表一行不退化。
- **⑦ X32 日历日有效**：`asof` 过词法还要过 `date.fromisoformat`，`2026-02-30` 单码 `DD-EVIDENCE`；只判形制，不判事件真伪（与 B17 词法题分开不合并）。
- 夹具：主集升 **60 坏 + 10 正路 = 70**、控制样 **19 枚**，题单 60 题 ↔ 60 样双向断言通过，幂等两跑夹具树 hash 零差异。逐题双向反证用 `gtm/scripts/dual_gate.py`（同一份夹具分别跑改前/改后两道门，工具落内部仓不落 /tmp）：**14 枚新样里 13 枚旧门 exit 0 且零诊断（洞在，与审方 /tmp 副本实测同向）**，`good-X29` 旧门 exit 1（过严误杀）；新门 14/14 与题面声明的精确码集相符。真实两笔不退化：m79 exit 0、m88 仍单码 `DD-COVERAGE`。
- §6 码表两行、§7 第 11 格同步：码表行是纯追加（旧行文本整段保留在新行内，已逐字核过），§7 只加"围栏示例须人工确认非真账"一格，机器面不扩。

### Fixed（M99 统一标题级口径，版本位不动）

- **§v2-14 一把尺**：新增 `heading_level()` / `ends_section()`，`COVERAGE_HEADING` 收严到 **H2–H4**——H1 是文档标题，永不作覆盖率节、不参与同名节计数；H5/H6 是节内附注，**不结束当前节、不缩小范围、不自成新节**。节内取行的循环从"任何 ATX 标题即 break"改为"只 H2–H4 结束"，所以附注之后的申报照样计入本节、附注之后再塞第二条也照样被计。
- 两题不同根、两处不同改：`bad-X24-h5-insection-duplicate`（同节 H5 后第二条合法申报，题面旧门实测 exit 0，本轮用 HEAD 版门复现同结果；改后 exit 1 单码 `DD-COVERAGE`）治的是**节范围截断**；`bad-X25-h1-title-masquerade`（H1 含「未检到」冒充节，旧门 exit 0、改后 exit 1 单码）治的是**级别误入**。正路配对 `good-X24-sectioned-h5-before-declaration`（H5 之后唯一合法申报，旧门被误杀 exit 1、改后 exit 0）——防"见 H5 一律拒产"式假修复。
- 合同升 **53 坏 + 9 正路 = 62 主集**全 ok，控制样 13 枚不变，双向断言 53↔53 通过，幂等两跑零差异；真实两笔与紧凑形不退化（m79 exit 0、m88 仍单码 DD-COVERAGE、59→62 全绿内含 compact 正路样）。
- 超出点名范围的两处同根因（按"任何一处再自判级别=同一 bug 复发"处理，报你裁）：①表节主体回溯原来在任意 ATX 标题处停住，现跳过 H5/H6 继续上溯真节标题；②多主体计数原来用 `^#{1,3} ` 正则（既不容 0–3 前导空格、又把 H5 拉进来），现统一走 `heading_level()` 取 H1–H4。另把 `ti = startswith("# ")`（找文档标题）也换成同一识别器——三空格缩进的 H1 之前会被判"无 H1 标题行"。
- 本刀起自用一条硬规矩（前两次吞邻居内容的代价）：**每次多行替换后必查 git diff 的删除行**，确认只删掉本应改写的行；本轮两次 SKILL 编辑都过这道核。

### Fixed（M97 封「同名节遮蔽」整类，版本位不动）

- **§v2-13 节计数取代节查找**：新增 `coverage_head_count()`，全文同名覆盖率节**必须恰为 1**——≥2 直接 `DD-COVERAGE`，不做"取首节"的歧义解决；0 个仍走 §v2-12 三态（compact 可全文扫 / missing 不回退全文）。这条把"跨节重复 / 首节遮蔽"整类一次封掉，以后同类新形状不需要再逐形加门。
- **同时修掉我自己 M95 引入的一处过严**：`sectioned` 判定当时写成"任何非 H1 标题"，把 **H5/H6 也当分节**，于是"整篇无 H2-H4、文末加 `##### 附注`"的合法紧凑形被误判缺节。现按 §v2-12 原口径只认 **H2–H4** 为分节（H5/H6 属节内附注）。
- 题库升 **51 坏 + 8 正路 = 59**：`bad-X23-duplicate-coverage-sections`（两个同名节各带一条合法申报，唯一坏点=跨节重复，精确码集 `DD-COVERAGE`）+ 正路守护 `good-X23-compact-h5-note`（紧凑形 + H5 附注，须 exit 0，专防上面那条过严回归）。控制样按审方 M96-A 判补到 **13 枚**：新增 `ctl-age-sectioned-declared`（分节且申报落在被识别节内，exit 0——"新门不恒红"的持久正路形，此前只在回执里活过）与 **`ctl-age-cap-overclaim`（上限写高 730d，exit 1；M94 就要求过的持久样，本轮不再欠）**。
- 反证三向（C-15/C-25，全部实测，取 `git show HEAD:` 版门做"改前"）：X23 形状**改前 exit 0（洞在，八道全过）→ 改后 exit 1、单码 `DD-COVERAGE`**；H5 紧凑正路形**改前 exit 1（我的过严 bug）→ 改后 exit 0**；真实两笔不退化——m79（12m/investment）exit 0、m88（24m/credit）仍 exit 1 单码。59 主集 + 13 控制样全绿，幂等两跑零差异。
- 一处自曝（同类第三次）：改 SKILL §5 时我的替换串把末段「为什么是 365 天」整段吞了，当场用 `git diff` 查非预期删除才发现并已复原（现该段计数=1，§5 四段齐：申报行式样 → 落点 §v2-12 → 同名节计数 §v2-13 → 为什么 365 天）。规格里长出来的教训：**多行替换后必须 diff 查删除行**，这已是本会话第二次我吞掉邻居内容（前一次是 channel-map 的边界 bullet）。

### Fixed（M95 修「节定位静默回退」，版本位不动）

- **§v2-12 三条落地**：①标题识别改按 CommonMark 接受 **0–3 个前导空格**的 ATX 形制（新增 `ATX_HEADING`/`is_heading`），覆盖率节与表节主体回溯两处共用同一识别器——只认 `startswith("#")` 会让缩进标题隐身；②节导航分三态：`section`（定位到该节，只在节内收行）/ `compact`（整篇无 H2-H4 的紧凑交付，允许全文扫）/ `missing`（有分节却没找到覆盖率节——**禁止回退全文**，直接判该节缺失 = `DD-COVERAGE`）；③申报计数只限覆盖率节内，申报写在别节＝本节缺失。
- 题单升 **50 坏 + 7 正路 = 57 主集**，新增 `bad-X22-section-lookup-bypass`（三空格标题 + 申报串到「其他说明」节），精确码集 `DD-COVERAGE`；控制样 11 枚按审方判定不删。双向反证（C-15/C-25）：X22 样在**改门前 exit 0（洞在，用 HEAD 版门跑）**、**改门后 exit 1 单码**；同时拿真实 24m run 造一份"申报行放在被识别的覆盖率节内"的分节合法形，改门后仍 **exit 0**——新逻辑没有把合法形状打成恒红。
- `coverage_section_lines()` 的返回从 list 改为 tuple（正文行 + 形态），两处调用点与表节主体回溯一并改为共用识别器；SKILL §5 补「申报行必须落在被识别的覆盖率节内、节定位失败不回退全文」一句。

### Fixed（M93 复审三 blocker，版本位不动）

- **B1「先收后判」**：`check_age_declaration()` 原来只把能被完整语法解析的行计入 `decls`，于是第二行把单位写成「日」这类字形变体既躲过解析、又不被计数——双申报行仍出门。现按契约改正：先按**归一前缀**收集覆盖率节内**全部**原始申报行 → 行数须恰为 1 → 再对这唯一一行做完整语法核验。反证位（C-15/C-25 双向）：同一份"一行合法 + 一行 730日/已覆盖"的临时副本，**改门前 exit 0（洞在此）**、**改门后 exit 1 且单码 `DD-COVERAGE`**，诊断行点名伪行原文。
- **B2 题库升 56**：`DD-X20`（beyond 洗成「零条」）与 `DD-X21`（一真一伪两行、伪行避开严格解析）按审方题号转正为 `bad-X20-beyond-laundered` / `bad-X21-double-declared-variant`，`expect.txt` 精确码集 `DD-COVERAGE`；与 X20 重复的控制样已收掉，另留题单许可的等价攻击与防死码控制样。现值：**主集 49 坏 + 7 正路 = 56 全 ok**，控制样 11 枚全 ok（比派单口径多 1 枚：`ctl-age-single-malformed` 专打"唯一一行即伪行"分支，不留死码）。生成器双向断言通过，幂等复跑零差异。
