...
| [`competitive-brief/`](competitive-brief/) | competitive-brief | 0.2.0 | 竞品简报生成器：把散落在网页 / PDF / 录音 / 视频里的竞品素材，变成可溯源、可对比、可更新的决策简报。六阶段管线：素材摄入（omni-reader）→ 需求对齐 → 对比框架 → 取证（cue-research）→ 三件套出稿 → 门禁校订。 |
| [`cn-earnings-note/`](cn-earnings-note/) | cn-earnings-note | 0.1.0 | A股 / 港股财报深度点评生成器：输入「主体 + 报告期」，产出 8–12 页、**研报结构级（八节骨架）**的 AI 初稿（四期披露差异、分部量价、盈利质量与现金流含金量、附注风险扫描、指引与催化剂、同业对照），逐数可回查，评级与目标价一律标注 `[待人工]`。取证走 Cue 三层通道——结构化披露（Cue 数据 MCP 域）+ 原文解析（omni-reader）+ 横向深研（cue-research）。**P1 端到端实测完成前，上述性能数字均为设计值。** 基座出处见 [NOTICE.md](NOTICE.md) §金融套件。 |
...
> **状态：** 以下**只有尖兵 `cn-earnings-note` 在本仓中真实存在**；其余均为已定方向、**尚未构建、不可用**。**P1 端到端实测完成前，任何性能数字均为设计值。** 除上表 Skills 所列成品外，本路线图不承诺任何已交付内容。

### 已建 —— 尖兵（1 件）

- **财报深度点评 —— [`cn-earnings-note/`](cn-earnings-note/)**（v0.1.0）：A股 / 港股财报深度点评，目前唯一已交付的金融 skill。完整一句话见上表 Skills。
